...
import { decodeLog, decodeMethod, fetchTransaction, getContractBehindProxy, DecodedLogResult, FailedDecodedLogResult, getTokenTransactionsFromAddressAfterBlock } from '../api/rpc';
...
export const fetchMethodInformationByTransactionHash = async (transactionHash: string, provider: ethers.JsonRpcProvider, chain: ChainInfo) => {
  const rootTransactionDetails: TransactionContext = await fetchTransaction(transactionHash, provider);
  const rootAddress = await getContractBehindProxy(rootTransactionDetails.to.address, provider)
  const contractAbi = rootAddress ? await fetchContractAbi(rootAddress, chain) : await fetchContractAbi(rootTransactionDetails.to.address, chain)
  return await decodeMethod(transactionHash, contractAbi, provider)

}
export interface DecodedEvents { decodedLogs: DecodedLogResult[], failedDecodedlogs: FailedDecodedLogResult[] }
export const fetchEventInformationByTransactionHash = async (transactionHash: string, provider: ethers.JsonRpcProvider, chain: ChainInfo): Promise<DecodedEvents> => {
  const rootTransactionDetails: TransactionContext = await fetchTransaction(transactionHash, provider);
  const decodedLogs = []
  ...
  for (const l of rootTransactionDetails.receipt.logs) {
    ...
    const contractAbi = rootAddress ? await fetchContractAbi(rootAddress, chain) : await fetchContractAbi(l.address, chain)
...
export type TransactionNativeType = 'nativeTransfer' | 'directTransfer' | 'sum' | 'split' | 'end' | 'root'
export type TransactionContractType = 'contractCall' | 'end' | 'transfer' | 'method' | 'root'
export interface FetchNativeTransaction { transactionType: TransactionNativeType[], transactionContext: TransactionContext }
export interface FetchContractTransaction { transactionType: TransactionContractType[], transactionContext: TransactionContext, events: DecodedEvents, method: DecodedMethodResult }

export const fetchTransactionInformation = async (params: { transactionHash: string, }, provider: ethers.JsonRpcProvider, chain: ChainInfo): Promise<FetchContractTransaction | FetchNativeTransaction> => {
  ...
  const transactionContext: TransactionContext = await fetchTransaction(params.transactionHash, provider);
  let payload: FetchNativeTransaction | FetchContractTransaction = { transactionContext, transactionType: ['nativeTransfer'] }
  if (transactionContext.to.type === 'contract') {
    const method = await fetchMethodInformationByTransactionHash(params.transactionHash, provider, chain)
...
function sumTransactions(transactions: Array<TransactionDetails>, targetTransaction: TransactionDetails) {
...
function splitTransaction(transactions: Array<TransactionDetails>, targetTransaction: TransactionDetails) {
...
export const fetchTransactionInteractionInformation = async (params: {
  ...
  const transactionsForAddress = await fetchTransactionForAddress(res.transactionContext.to.address, endBlock, chain)
  const nativeTransactions: FetchNativeTransaction[] = []
  const contractTransactions: FetchContractTransaction[] = []
  for (const t of transactionsForAddress) { // sort transactions by event type 
    if (params.transactionHash === t.hash) {
      continue
    }
    const transactionInformation = await fetchTransactionInformation({ transactionHash: t.hash }, provider, chain);
    if (transactionInformation.transactionType[0] === 'nativeTransfer') {
      ...
      const possibleDirectTransfer = await isNativeTokenTransferWithinRange(Number(res.transactionContext.formattedValue), Number(transactionInformation.transactionContext.formattedValue), 200, chain)
      if (possibleDirectTransfer) {
        nativeTransactions.push({ ...nativeTransaction, transactionType: [...nativeTransaction.transactionType, 'directTransfer'] })
      }
      const split = splitTransaction(transactionsForAddress, t)
      if (split?.length > 0) {
        nativeTransactions.push({ ...nativeTransaction, transactionType: [...nativeTransaction.transactionType, 'split'] })

      }
      const sum = sumTransactions(transactionsForAddress, t)
      if (sum.length) {
        nativeTransactions.push({ ...nativeTransaction, transactionType: [...nativeTransaction.transactionType, 'sum'] })
      }
    }
    ...
    if (transactionInformation.transactionType[0] === 'contractCall') {
      ...
      if (contractTransaction.method.methodName === 'Transfer') {
        contractTransactions.push({ ...contractTransaction, transactionType: [...contractTransaction.transactionType, 'transfer'] })
      } else {
        contractTransactions.push({ ...contractTransaction, transactionType: [...contractTransaction.transactionType, 'method'] })
      }
    }
